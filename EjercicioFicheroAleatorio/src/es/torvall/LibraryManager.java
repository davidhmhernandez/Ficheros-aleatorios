...
public class LibraryManager {
			...
			while ((line = streamIn.readLine()) != null) {// Leemos el Archivo
				...
				try { // Hasta el Final
					...
					while (st.hasMoreTokens()) {// se lee las lineas mientras
						...
						// se crea el objeto libro b = new Book(id, title,
						// fk_author, fk_publisher, year,stock);
						libros.add(b);
	...
	public Book findBook(int id, String randomFile) {
		...
		try {
			...
			file.seek(posicion);
			if (file.read() == id) {
				int book_id = file.readInt();
				file.skipBytes(4);
				StringBuilder sb = new StringBuilder();
				char letra;
				for (int i = 0; i < TAM_NAME; i++) {
					letra = file.readChar(); // Leemos caracter a caracter
				...
				String title = sb.toString();
				file.skipBytes(40);
				int fk_author = file.readInt();
				file.skipBytes(4);
				int year = file.readInt();
				file.skipBytes(4);
				int fk_publisher = file.readInt();
				file.skipBytes(4);
				int stock = file.readInt();
				libro = new Book(book_id, title, fk_author, year, fk_publisher,
						stock);
			}
	...
	public ArrayList<Book> buscarLibros(String randomFile, int editorial) {
		...
		// Forma lenta
		/*
		 * try { streamIn = new RandomAccessFile(randomFile, "r"); do { book =
		 * readBook(streamIn); if (book.getFk_publisher() == editorial)
		 * libros.add(book); } while (streamIn.getFilePointer() !=
		...
		try {
			...
			do {
				...
				streamIn.seek(posicion); // Nos posicionamos en la editorial
											// directamente
				...
				if (publisher == editorial) { // Si son iguales...
					posicion -= POSICION_EDITORIAL; // Nos colocamos al inicio
													// del libro
					streamIn.seek(posicion); // Nos colocamos al inicio del
												// libro
